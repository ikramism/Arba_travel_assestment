from fastapi import FastAPI, Request, HTTPException, Depends, Header, Form, UploadFile, Query
...
# set loginPage.html as main Page
@app.get("/", response_class=HTMLResponse)
async def get_login_page():
    try:
        # Serve the LoginPage.html from the Front-end folder
        with open(os.path.join("Front-end", "LoginPage.html"), "r") as f:
            return HTMLResponse(content=f.read())
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail="LoginPage.html not found")
